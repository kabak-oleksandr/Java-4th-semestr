...
public class Menu {
    ...
    static LinkedList<Store> stores = new LinkedList<Store>();
    ...
    private void readInfoFile() {
        try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream("C:\\Greed\\Study\\Java\\Lab9\\person.dat"))) {
            stores = (LinkedList<Store>) ois.readObject();
    ...
    private String regSpecialization() {
        String tmp = in.nextLine();
        while (!tmp.matches("(^[A-Z])(\\D)([a-zA-Z[ -]]+)")) {
            System.out.println("Введите название специализации корректно(с заглавной,без знаков(допускается \"-\"и \" \"))");
            tmp = in.nextLine();
        }
        return tmp;
    }
